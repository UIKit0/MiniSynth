...
public class FixedFreqSine {
	...
	public static void main(String[] args) throws InterruptedException,
		...
		// Position through the sine wave as a percentage (i.e. 0 to 1 is 0 to
		// 2*PI)
		...
		while (ctSamplesTotal > 0) {
			double fCycleInc = fFreq / SAMPLING_RATE; // Fraction of cycle
														// between samples
			...
			for (int i = 0; i < ctSamplesThisPass; i++) {
				cBuf.putShort((short) (Short.MAX_VALUE * Math.sin(2 * Math.PI
						* fCyclePosition)));
			...
			// Write sine samples to the line buffer. If the audio buffer is
			// full, this will
			// block until there is room (we never write more samples than
			// buffer will hold)
			line.write(cBuf.array(), 0, cBuf.position());
			ctSamplesTotal -= ctSamplesThisPass; // Update total number of
													// samples written
		...
		// Done playing the whole waveform, now wait until the queued samples
		// finish
